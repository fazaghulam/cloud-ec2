...
export const Todo = () => {
  ...
  return !isLoading ? (
    ...
    >
      ...
      {showArr?.length
        ? Object.values(showArr).map((item, index) => (
            <div key={index} style={{ marginBottom: "10px" }}>
              ...
              <Button type="button" className="todoEditbtn" onClick={() => Edit(item?._id)} variant="secondary">
                {" "}
                Edit{" "}
              </Button>
              <Button variant="danger" className="todoDeletebtn" type="button" onClick={() => Remove(item?._id)}>
                {" "}
                X{" "}
              </Button>
            </div>
      ...
      &nbsp;{" "}
      <Button onClick={() => DeleteAll()} className="deleteall" variant="danger">
        Delete All
      </Button>
    </form>
  ) : (
